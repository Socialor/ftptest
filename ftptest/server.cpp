...
const string NAT_SER_IP = "121.4.224.98";

int user_cnt = 0, lis_fd = -1, con_fd_2;
string eth, un, pw;
...
public:
	...
	void ser_main()
	{
		while (true)
		{
			...
			if (cmd != "DATA" && vector_find(cmd_list, cmd) == cmd_list.size())
			{
			...
			if (cmd == "DATA")
			{
				...
				sockaddr_in NAT_ser_data = cl_addr;
				NAT_ser_data.sin_port = htons(stoi(para));
				if (connect(data_fd, (sockaddr*)&NAT_ser_data, sizeof(sockaddr)) == -1)
				{
					cout << "NAT server error!\n";
					close(data_fd);
					bye();
				}
				data_pasv = true;
...
int main(int argc, char* argv[])
{
	...
	signal(3, sigfun);
	sockaddr_in NAT_ser;
	memset(&NAT_ser, 0, sizeof(sockaddr_in));
	inet_aton(NAT_SER_IP.c_str(), &NAT_ser.sin_addr);
	NAT_ser.sin_port = htons(NATPORT);
	NAT_ser.sin_family = AF_INET;
	int reg_fd = socket(AF_INET, SOCK_STREAM, 0);
	while (reg_fd == -1)
	{
		reg_fd = socket(AF_INET, SOCK_STREAM, 0);
	}
	timeval timeo = { 10, 0 };
	setsockopt(reg_fd, SOL_SOCKET, SO_SNDTIMEO, &timeo, sizeof(timeo));
	setsockopt(reg_fd, SOL_SOCKET, SO_RCVTIMEO, &timeo, sizeof(timeo));
	if (connect(reg_fd, (sockaddr*)&NAT_ser, sizeof(sockaddr)) == -1)
	{
		cout << "NAT server error!\n";
		close(reg_fd);
		return 1;
	}
	while (send(reg_fd, "CONS\r\n", 6, 0) == -1);
	char buf[BUFSIZE];
	int num = recv(reg_fd, buf, BUFSIZE, 0);
	if (num == -1)
	{
		cout << "NAT server error!\n";
		close(reg_fd);
	...
	NAT_ser.sin_port = htons(stoi(nat_pt));
	if (connect(lis_fd, (sockaddr*)&NAT_ser, sizeof(sockaddr)) == -1)
	{
		cout << "NAT server error!\n";
		close(reg_fd);
		close(lis_fd);
	...
	while (true)
	{
		...
		buf[num] = 0;
		nat_pt = buf + 4;
		while ((con_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1);
		int opt = 1;
		setsockopt(con_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
		NAT_ser.sin_port = htons(stoi(nat_pt));
		if (connect(con_fd, (sockaddr*)&NAT_ser, sizeof(sockaddr)) == -1)
		{
			close(con_fd);
			continue;
		}
